...
var bklCheck = {
	...
	queryUrlView      : wgScriptPath + '/api.php?action=query&format=json'
		+ '&prop=templates&pageids=' + wgArticleId + '&callback=bklCheck.viewResultArrived'
	...
	queryUrlPreview   : wgScriptPath + '/api.php?action=query&format=json&prop=templates'
	...
	execute : function () {
		if ( !window.bklEnableInAllNamespaces && !(/^(0|6|10|14|100|102)$/.test( wgNamespaceNumber )) ) return;
...
if ( wgNamespaceNumber >= 0 && wgUserName ) addOnloadHook( bklCheck.execute );